...
//const httpio = require('http').Server(exp);
...
oscServer.on("message", function (msg, rinfo) {                // Process incoming local OSC messages
  oscSend(msg[0], msg[1], msg[2], msg[3], msg[4]);
  rendererWin.webContents.send('oscReceive', msg[0], msg[1], msg[2], msg[3]);
  console.log("OSC message came in! Message: "+msg[0]+", Parameters: "+msg[1]+msg[2]+msg[3]);
});

function oscSend(mes, param1, param2, param3, param4){                         // Forward websocket to tablet-receivers

  var messObject = {                         // store the data in an JSON Object
      "message": mes,
      "parameter1": param1,
      "parameter2": param2,
      "parameter3": param3,
...
server.listen(serverPort, function() {                                 // After server was started, send general information to console
...
function createWindow () {
  rendererWin = new BrowserWindow({width: 640, height: 480, webPreferences: {nodeIntegration: true} }) // Erstelle das Browser-Fenster.
